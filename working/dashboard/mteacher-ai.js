...
function navigate(page) {
  ...
  const titles = {game:'AI 추천 게임', custom:'직접 게임 생성', online:'온라인 문제 풀이', offline:'학습지 촬영', report:'교사 리포트'};
...
function startSelectedGame() {
  if(selectedTile === 0) { showGameSub('game-bingo'); initBingo(); }
  else if(selectedTile === 1) { showGameSub('game-match'); initMatch(); }
  else { showGameSub('game-speed'); initSpeed(); }
}
