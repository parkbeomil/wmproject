...
const SYSTEM = `당신은 초등학교 ${grade}학년 ${semester}학기 수학 중 "${unit}" 단원을 가르치는 소크라테스식 선생님입니다.
...
... (중략: 기존의 언어/안전/감정/말투 규칙 유지) ...
`;

// 실제로는 중략된 부분을 포함한 전체 SYSTEM 프롬프트를 유지해야 함. 
// 아래에서 전체 코드를 다시 작성함.
...
function initialGreeting() {
  ...
  if (extractedProblem) {
    greeting += `<br><br>가져온 문제를 확인해봤어! 전문가는 아니지만 같이 해결해보자.<br>
  ...
  if (extractedProblem) {
    ...
    chipsContainer.innerHTML = `
      <span class="chip" onclick="setInput('${unit}이(가) 뭐예요?')">${unit}이(가) 뭐예요?</span>
...
document.getElementById('reset-key').addEventListener('click', () => {
  ...
  sessionStorage.removeItem('study_grade');
  sessionStorage.removeItem('study_semester');
  sessionStorage.removeItem('study_unit');
  window.location.href = 'auth.html';
});
...
async function send() {
  ...
  // 완성된 SYSTEM 프롬프트 생성
  let FULL_SYSTEM = `당신은 초등학교 ${grade}학년 ${semester}학기 수학 중 "${unit}" 단원을 가르치는 소크라테스식 선생님입니다.
대화 상대는 초등학교 ${grade}학년 학생입니다.
선생님 이름은 "미래엔 소크라테스 선생님"입니다.
